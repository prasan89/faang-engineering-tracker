...
export default function Home(){
 ...
 const toggle=(id:string,v:boolean)=>setCompleted(x=>({...x,[id]:v}));
 ...
 <section className="grid"><div className="card wide"><h2>Today</h2><p className="muted">{today.theme} · {today.focus}</p>{today.tasks.map(t=><label className="task" key={t.id}><input type="checkbox" checked={!!completed[t.id]} onChange={e=>toggle(t.id,e.target.checked)}/><span><b>{t.title}</b><small>{t.track} · {t.minutes} min</small></span></label>)}</div>
 ...
 <div className="card"><h2>Recognition pipeline</h2><div className="pipeline"><span>GitHub projects</span><span>Technical writing</span><span>Open-source PRs</span><span>Networking</span><span>Referrals</span><span>Applications</span></div></div></section><footer>Next: connect PostgreSQL + authentication + GitHub Actions + Vercel deployment.</footer></main>
}